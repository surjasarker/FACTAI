...
hf_modules_path = os.path.expanduser("~/.cache/huggingface/modules")
if hf_modules_path not in sys.path:
    sys.path.append(hf_modules_path)

# Also add the specific folder where cvlface was downloaded
cvl_path = os.path.expanduser("~/.cache/huggingface/modules/transformers_modules/cvlface_adaface_vit_base_kprpe_webface4m")
if cvl_path not in sys.path:
    sys.path.append(cvl_path)
...
import torchvision
import lpips
from utils_flux import load_model_by_repo_id, pil_to_input
from methods_robust import cw_l2_attack, vae_attack, encoder_attack, facelock
import argparse
...
try:
    from transformers_modules.cvlface_adaface_vit_base_kprpe_webface4m.wrapper import CVLFaceRecognitionModel
    if not hasattr(CVLFaceRecognitionModel, "all_tied_weights_keys"):
        CVLFaceRecognitionModel.all_tied_weights_keys = property(
...
def get_args_parser():
    ...
    parser.add_argument("--target_model", default="Flux", type=str, help="the target image editing model [instruct-pix2pix/stable-diffusion/Flux.1-Kontext]")
