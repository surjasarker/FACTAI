...
## Abstract
Instruction-based image editing techniques pose major privacy risks. Users with malicious intent can edit portrait images without an individual's consent. This enables impersonation, creating deepfakes, and engaging in targeted harassment. This paper reproduces FaceLock, a Projected Gradient Descent (PGD) Defense method, aiming to create biometrically unrecognizable outputs. While previous methods focus on disrupting the latent space of the VAE, Facelock utilizes facial recognition and visual feature disparity losses to remove biometric information. It creates perturbations using the VAE of the InstructPix2Pix model. We successfully reproduced the Facelock paper, achieving similar FR, CLIP-I, LPIPS, SSIM, PSNR, and CLIP-S values. The only modification required was saving the defended images in PNG format instead of JPG, as the original code saves outputs using the original CelebA-HQ filenames (with .jpg extensions), and JPEG compression can attenuate or remove the learned perturbations. In addition, we extended their work to successfully demonstrate FaceLock's generalization to the larger state-of-the-art FLUX.2-klein-9B editing model.
## Environments
